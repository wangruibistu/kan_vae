...
from train_fns import weighted_masked_mse_loss, CSNDataset, batch_to_cuda, \
    train_iter, val_iter, EarlyStopping
from model import KAN_VAE

os.environ['CUDA_VISIBLE_DEVICES'] = '1'
model_name = 'marcs_to_lamostmrs_90'#
...
verbose_iters = 1000  # args.verbose_iters
cp_time = 15  # args.cp_time
data_dir = '/home/wangr/python/parameters/medres_parameter/mrs_Cycle_SN/data/'  # args.data_dir
cur_dir = '/home/wangr/python/parameters/medres_parameter/mrs_Cycle_SN/'
# Directories
...
lr_scheduler_rg = torch.optim.lr_scheduler.MultiStepLR(optimizer_rec_and_gen, milestones=lr_decay_batch_iters_rg,
                                                       gamma=lr_decay_rg)

lr_scheduler_dis = torch.optim.lr_scheduler.MultiStepLR(optimizer_dis, milestones=lr_decay_batch_iters_dis,
                                                        gamma=lr_decay_dis)
# Loss functions
...
obs_train_dataset = CSNDataset(data_file_obs, dataset='train', x_mean=x_mean, x_std=x_std,
                               line_mask=None)
obs_train_dataloader = DataLoader(obs_train_dataset, batch_size=batchsize,
                                  shuffle=True, num_workers=8)

synth_train_dataset = CSNDataset(data_file_synth, dataset='train', x_mean=x_mean,
                                 x_std=x_std, line_mask=None)
synth_train_dataloader = DataLoader(synth_train_dataset, batch_size=batchsize,
                                    shuffle=True, num_workers=8)

# Validation set that consists of matching pairs in the synthetic and observed domains
obs_val_dataset = CSNDataset(data_file_obs, dataset='val', x_mean=x_mean, x_std=x_std,
                             line_mask=None)
obs_val_dataloader = DataLoader(obs_val_dataset, batch_size=128, shuffle=True,
                                num_workers=8)
...
def train_network(cur_iter):
    ...
    while cur_iter < total_batch_iters:
        ...
        for obs_train_batch in obs_train_dataloader:
            ...
            # Train an iteration
            # print(obs_train_batch['x'].shape)
            # print(synth_train_batch['x'].shape)
            losses_cp = train_iter(model=model,
                                   obs_train_batch=obs_train_batch,
                                   synth_train_batch=synth_train_batch,
                                   distance_loss=distance_loss,
                                   gan_loss=gan_loss,
                                   loss_weight_synth=loss_weight_synth,
                                   loss_weight_obs=loss_weight_obs,
                                   loss_weight_gen=loss_weight_gen,
                                   loss_weight_dis=loss_weight_dis,
                                   optimizer_rec_and_gen=optimizer_rec_and_gen,
                                   optimizer_dis=optimizer_dis,
                                   lr_scheduler_rg=lr_scheduler_rg,
                                   lr_scheduler_dis=lr_scheduler_dis,
                                   use_real_as_true=use_real_as_true,
                                   losses_cp=losses_cp,
                                   use_cuda=use_cuda)
            ...
            if cur_iter % verbose_iters == 0:
                ...
                while i < 10:
                    for obs_val_batch in obs_val_dataloader:
                        ...
                        losses_cp = val_iter(model, obs_val_batch, x_mean, x_std, distance_loss, losses_cp,
                ...
                print('\n')
                print('val loss synthobs bias: %.4f'%losses['bias_val'][-1])
                print('val loss synthobs scatter: %.4f'%losses['scatter_val'][-1])
                print('\n')
                early_stopping(losses['scatter_val'][-1], model, cur_iter, losses,
                               optimizer_rec_and_gen, optimizer_dis, lr_scheduler_rg,
