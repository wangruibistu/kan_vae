...
#                 ] += self.kan_conv2d(input[:, :, i : i + 1, j : j + 1])
...
#         input_unf = input_unf.view(
#             batch_size, self.in_channels, -1, height, width
#         ).permute(0, 2, 1, 3, 4)

#         # Apply KANConv2d
#         output_unf = self.kan_conv2d(input_unf.reshape(-1, self.in_channels, 1, 1))
...
#             + self.output_padding[1]
#         )
#         # Fold the output tensor back
#         output_unf = output_unf.view(batch_size, -1, height, width)
#         output = F.fold(
#             output_unf,
