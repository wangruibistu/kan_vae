...
class KANLinear2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        spline_output = F.conv2d(
            self.b_splines(x).view(x.size(0), -1, x.size(2), x.size(3)),
            self.scaled_spline_weight.view(
    ...
    def update_grid(self, x: torch.Tensor, margin=0.01):
        ...
        splines = splines.permute(1, 0, 2, 3, 4)
        orig_coeff = self.scaled_spline_weight
        orig_coeff = orig_coeff.permute(1, 2, 3, 4, 0)
        unreduced_spline_output = torch.bmm(
            splines.view(splines.size(0), -1, splines.size(-1)),
            orig_coeff.view(orig_coeff.size(0), -1, orig_coeff.size(-1)),
        )
        unreduced_spline_output = unreduced_spline_output.view(
            splines.size(1),
            splines.size(2),
            splines.size(3),
            splines.size(4),
