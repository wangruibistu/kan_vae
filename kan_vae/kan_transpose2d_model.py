...
import math
import warnings
# from kan_model import KANLinear
...
class KANLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        spline_output = (
            torch.bmm(
                self.b_splines(x).view(x.size(0) * x.size(1), -1).unsqueeze(0),
                self.scaled_spline_weight.view(self.groups, self.out_features, -1)
                .permute(0, 2, 1)
                .unsqueeze(0),
            )
            .squeeze(0)
            .view(x.size(0), x.size(1), self.out_features)
        )
...
class KANConvTranspose2d(nn.Module):
    def __init__(
        ...
        """
        Transposed Convolutional layer with KAN kernels. A drop-in replacement for torch.nn.ConvTranspose2d.
        ...
        Args:
            ...
            output_padding (int or tuple): Additional size added to one side of each dimension in the output shape. Default: 0
            dilation (int or tuple): Spacing between kernel elements. Default: 1
            groups (int): Number of blocked connections from input channels to output channels. Default: 1
            bias (bool): Added for compatibility with torch.nn.ConvTranspose2d and does make any effect. Default: True
            ...
            base_activation (nn.Module): Activation function for the base. Default: torch.nn.SiLU
    ...
    def forward(self, x):
        ...
        # Reshape the unfolded tensor to match the input requirements of the linear layer
        x_unf = x_unf.view(
            batch_size,
            self.groups,
